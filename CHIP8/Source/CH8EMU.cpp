#include "CH8CPU.hpp"
#include "CH8EMU.hpp" // Fully included to resolve cross-subsystem references cleanly

CH8EMU::CH8EMU() : CH8COM(this), bus(this), cpu(this), ppu(this), con(this), apu(this), dsk(this) CH8_DBG_INIT(dbg(this)) {
    reset();
}
...
bool CH8EMU::injectROM(const Uint8* data, size_t size) {
    if (!data || size == 0 || size > 3584) return false;
    SDL_memcpy(&bus.RAM[0x200], data, size);
    return true;
}

// CH8COM bus access wrapper implementations
...
void CH8CPU::powerOn() { reset(); }
void CH8CPU::powerOff() { reset(); }

void CH8CPU::reset() {
    I = 0; PC = 0x200; SP = 0; DELAY_TIMER = 0; SOUND_TIMER = 0; currentOpcode = 0;
...
void CH8CPU::execute() {
    // Single line macro handles analytical runtime inspection streams natively
    CH8_DBG_EXEC(SDL_Log("Trace: PC: 0x%04X | Op: 0x%04X | %s", PC - 2, currentOpcode, matchedInstruction.mnemonic));
